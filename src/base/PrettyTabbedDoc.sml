...
struct
  ...
  let
    ...
    let
      val orderedHighlightCols =
        ...
        Seq.fromList (ListMergeSort.sort (Util.gtOfCmp sentryCmp) startDebugs)
      ...
      if List.null endDebugs then
        ...
        let
          ...
          val orderedEnds =
            ...
            Seq.fromList (ListMergeSort.sort (Util.gtOfCmp eentryCmp) endDebugs)
    ...
    fun processItem (item, (accCurrLine, acc, endDebugs, startDebugs)) =
      ...
      | Item.Newline =>
          let
            val (remainingSS, acc) = newlineWithEndDebugs endDebugs startDebugs
    ...
    val (accCurrLine, acc, endDebugs, startDebugs) = List.foldr processItem init
  ...
  let
    ...
    fun dbgInsert tab
      (LS (dbgState, ct, cats, coms, s, c, sp, a) : layout_state) : layout_state =
      if not debug then
        LS (dbgState, ct, cats, coms, s, c, sp, a)
      else
        LS (TabDict.insert dbgState (tab, false), ct, cats, coms, s, c, sp, a)

    fun dbgBreak tab (LS (dbgState, ct, cats, coms, s, c, sp, a) : layout_state) :
      layout_state =
      if not debug then
        LS (dbgState, ct, cats, coms, s, c, sp, a)
    ...
    fun layout vars (state: layout_state) doc : layout_state =
      ...
      | NewTab {tab, gen} =>
          let
            ...
            val
              LS (dbgState, _, cats, coms, lnStart, col, sp, acc) : layout_state =
              doit ()
    ...
    let
      val LS (dbgState, ct, cats, coms, lnStart, col, sp, acc) = state
      val state = LS (dbgState, ct, cats, Seq.empty (), lnStart, col, sp, acc)
    ...
    val init = LS
      (TabDict.empty, root, TabSet.singleton root, initComments, 0, 0, true, [])
