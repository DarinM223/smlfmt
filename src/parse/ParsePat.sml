  ...
  let
      ...
      val (i, pat) =
        if isReserved Token.Underscore i then
          ...
          ParserUtils.tokError toks
            { pos = i
            , what = "Unexpected token. Expected to see beginning of pattern."
            , explain = NONE
            }
    in
    ...
    let
      val (again, (i, pat)) =
        ...
        then
          ( true
          , consume_patAs infdict (Ast.Pat.unpackForAsPat pat) (tok i) (i + 1)
          )
    ...
    and consume_patRow infdict i =
      ...
      else if check Token.isRecordLabel i andalso isReserved Token.Equal (i + 1) then
        let
          val (i, lab) = (i + 1, tok i)
          val (i, eq) = (i + 1, tok i)
    ...
    let val (i, pat) = consume_onePat infdict Restriction.None i
    in (i, Ast.Pat.Layered {opp = opp, id = id, ty = ty, ass = ass, pat = pat})
    end
