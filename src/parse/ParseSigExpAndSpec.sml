...
struct
  ...
  let
    ...
    let
      val (i, sigexp) =
        if isReserved Token.Sig i then consume_sigExpSigEnd (i + 1)
        else let val (i, sigid) = parse_sigid i in (i, Ast.Sig.Ident sigid) end
    in
  ...
  let
    ...
    fun consume_sigSpecTypeAbbreviation typee firstElem i =
      if not (isReserved Token.And i) then
        ...
        , Ast.Sig.TypeAbbreviation
            { typee = typee
            , typbind = {elems = Seq.singleton firstElem, delims = Seq.empty ()}
            }
        )
      else
        let
    ...
    let
      ...
      val (i, {elems, delims}) =
        parse_oneOrMoreDelimitedByReserved
          {parseElem = parseOne, delim = Token.And} i
    in
      (i, Ast.Sig.Eqtype {eqtypee = eqtypee, elems = elems, delims = delims})
    end
      ...
      val (again, (i, spec)) =
        if isReserved Token.Sharing i then
          if isReserved Token.Type (i + 1) then
            (true, consume_sigSharingType spec (tok i) (tok (i + 1)) (i + 2))
          else
    ...
    let
      ...
      fun makeSpecMultiple () =
        Ast.Sig.Multiple {elems = Seq.map #1 specs, delims = Seq.map #2 specs}
