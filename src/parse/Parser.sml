...
struct
  ...
  let
    ...
    let val (i, sigexp) = consume_sigExp infdict i
    in (i, Ast.Str.Constraint {strexp = strexp, colon = colon, sigexp = sigexp})
    end
    ...
    and consume_strexpFunApp infdict funid lparen i =
      ...
      then
        ...
        in
          ( i
          , Ast.Str.FunAppDec
              {funid = funid, lparen = lparen, strdec = strdec, rparen = rparen}
          )
        end
      else
        let
        ...
        in
          ( i
          , Ast.Str.FunAppExp
              {funid = funid, lparen = lparen, strexp = strexp, rparen = rparen}
          )
        end
    ...
    let
      ...
      let
        ...
        val (i, strexp) = consume_strexp infdict i
      in
        (i, {strid = strid, constraint = constraint, eq = eq, strexp = strexp})
      end
    ...
    let
      fun consume_maybeSemicolon (i, infdict) =
        if isReserved Token.Semicolon i then ((i + 1, infdict), SOME (tok i))
        else ((i, infdict), NONE)
    ...
    let
      ...
      fun parse_funarg infdict i =
        if not (check Token.isStrIdentifier i) then
          ...
          in
            ( i
            , Ast.Fun.ArgIdent {strid = strid, colon = colon, sigexp = sigexp}
            )
          end
  ...
  in
    if numToks > 0 then
      ...
      in
        if Seq.length commentsOnly > 0 then (infdict, JustComments commentsOnly)
        else (infdict, Ast (Ast.Ast topdecs))
      end
  end
