...
struct
  ...
  let
    ...
    let
      fun consume_maybeSemicolon (i, infdict) =
        if isReserved Token.Semicolon at i then ((i + 1, infdict), SOME (tok i))
        else ((i, infdict), NONE)
      ...
      (** While we see a dec-start token, parse pairs of
        ...
        *)
      val ((i, infdict), decs) =
        parse_zeroOrMoreWhile (fn (i, _) => check Token.isDecStartToken at i)
          (parse_two (consume_oneDec, consume_maybeSemicolon)) (i, infdict)

      fun makeDecMultiple () =
        Ast.Exp.DecMultiple {elems = Seq.map #1 decs, delims = Seq.map #2 decs}
    ...
    let
      ...
      val (i, precedence) =
        if check Token.isDecimalIntegerConstant at i then (i + 1, SOME (tok i))
        else (i, NONE)

      fun loop acc i =
        if check Token.isValueIdentifier at i then loop (tok i :: acc) (i + 1)
        else (i, Seq.fromRevList acc)
    ...
    and consume_decDatatypeDeclarationOrReplication (i, infdict) =
      ...
      then
        ...
        in
          ( (i, infdict)
          , Ast.Exp.DecDatatype
              {datatypee = datatypee, datbind = datbind, withtypee = withtypee}
          )
        end
      else
        let
  ...
  let
    ...
    in
      ( i
      , Ast.Exp.Fn {fnn = fnn, elems = elems, delims = delims, optbar = optbar}
      )
    end
    ...
    and consume_expParensOrTupleOrUnitOrSequence infdict leftParen i =
      if isReserved Token.CloseParen at i then
        ...
        in
          if isReserved Token.CloseParen at i then
            (i + 1, Ast.Exp.Parens {left = leftParen, exp = exp, right = tok i})
          else
            let
              val delimType =
