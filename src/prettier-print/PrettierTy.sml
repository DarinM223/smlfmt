...
struct
  ...
  in
    ...
    | Con {args = Ast.SyntaxSeq.Empty, id} => token (MaybeLongToken.getToken id)
    ...
    | Record {left, elems, delims, right} =>
        let
          fun showElem tab {lab, colon, ty} =
            token lab
            ++
            (if Token.isSymbolicIdentifier lab orelse Token.hasCommentsAfter lab then
               empty
             else
               nospace) ++ token colon ++ showTy tab ty
        in
