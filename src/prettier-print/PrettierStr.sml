  ...
  in
    ...
    | FunAppExp {funid, lparen, strexp, rparen} =>
        ...
        newTab tab (fn inner =>
          token funid
          ++
          (if strExpInsideFunAppWantsSpaceBefore strexp then space else nospace)
          ++
          at inner
    ...
    | FunAppDec {funid, lparen, strdec, rparen} => (* See note above, about the maybe-space after `funid` *)
        newTab tab (fn inner =>
          token funid
          ++
          (if strDecInsideFunAppWantsSpaceBefore strdec then space else nospace)
          ++
          at inner
        ...
        newTab tab (fn inner =>
          let
            val front =
              ...
              ++
              at inner
                (token ass ++ token (MaybeLongToken.getToken (Seq.nth elems 0)))
