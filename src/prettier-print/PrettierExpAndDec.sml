...
struct
    ...
    val rightNice =
      case right of
        Ident {opp = NONE, id} =>
          ...
          andalso not (Token.isSymbolicIdentifier (MaybeLongToken.getToken id))
      | Tuple _ => true
      | List _ => true
      | Sequence _ => true
      | Parens _ => true
  ...
  in
    ...
    | Select {hash, label} =>
        token hash
        ++
        (if Token.isSymbolicIdentifier label orelse Token.hasCommentsAfter hash then
           empty
         else
           nospace) ++ token label
        ...
        if Seq.length elems > 1 orelse Option.isSome optbar then
          ...
          newTab tab (fn inner =>
            ...
            in
              ...
              ++
              at tab (at inner
                (token handlee ++ withNewChild showPat inner pat ++ token arrow
                 ++ withNewChild showExp inner exp))
            end)
    ...
    if not (isSplittableExp exp) then
      ...
      | Fn {fnn, elems, delims, optbar} =>
          let
            val {pat, arrow, exp} = Seq.nth elems 0
          in
            token fnn ++ showOption token optbar ++ withNewChild showPat tab pat
            ++ token arrow
          end
  ...
  in
    ...
    | DecVal {vall, tyvars, elems, delims} =>
        let
          ...
          fun mkSimple (starter, {recc, pat, eq, exp}) =
            at tab
              (starter ++ showOption token recc ++ withNewChild showPat tab pat
               ++ token eq ++ withNewChild showExp tab exp)
    ...
    | DecMultiple {elems, delims} =>
        let
          fun mk first (elem, delim) =
            at tab
              (showDec tab elem ++ showOption (fn d => nospace ++ token d) delim)
    ...
    | DecOpen {openn, elems} =>
        token openn
        ++
        Seq.iterate op++ empty (Seq.map (token o MaybeLongToken.getToken) elems)
  ...
  let
    ...
    fun showClause mainTab clauseTab clauseChildStyleFirst clauseChildStyleRest
      isLone isFirst (front, {fname_args, ty, eq, exp}) =
    let
    ...
    let
      ...
      val mainStyle = Tab.Style.combine (rigidInplace, Tab.Style.allowComments)
