...
struct
  ...
  let
    ...
    fun makeSMLPath pathtok pathstr =
      MLBAst.DecPathSML {token = pathtok, path = FilePath.fromUnixPath pathstr}

    fun makeMLBPath pathtok pathstr =
      MLBAst.DecPathMLB {token = pathtok, path = FilePath.fromUnixPath pathstr}
    ...
    let
      val (i, elems) =
        ParserCombinators.oneOrMoreWhile (checkSML toks Token.isStrIdentifier)
          (parse_basid toks) i
    in
    ...
    in
      ( i
      , MLBAst.DecFunctor {functorr = functorr, elems = elems, delims = delims}
      )
    end
    ...
    let
      val (i, annotations) =
        ParserCombinators.oneOrMoreWhile (checkSML toks Token.isStringConstant)
          (parse_stringConstant toks) i
    ...
    let
      ...
      val (i, basdecs) =
        ParserCombinators.zeroOrMoreWhile continue
          (ParserCombinators.two (parse_exactlyOneDec, parse_maybeSemicolon)) i
