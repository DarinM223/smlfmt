...
struct
  ...
  let
    ...
    in
      if skipBasis andalso List.exists (fn k => k = "SML_LIB") used then
        ...
        let
          ...
          val (infdict, ast) =
            Parser.parseWithInfdict (#allows ctx) (#fixities basis) allTokens
      ...
      | DecAnn {basdec, annotations, ...} =>
          let
            val ctx' =
              { parents = #parents ctx
              , dir = #dir ctx
              , allows = ParseAnnotations.modifyAllows (#allows ctx) annotations
              }
          in
            doBasdec ctx' (mlbCache, basis, basdec)
          end
    ...
    fun topLevelError msg =
      raise Error.Error
        { header = "FILE ERROR"
        , content = [Error.Paragraph (msg ^ ": " ^ FilePath.toUnixPath mlbPath)]
        }
    ...
    val (_, _, asts) =
      doMLB
        {parents = [], dir = FilePath.fromUnixPath ".", allows = defaultAllows}
