...
struct
  ...
  let
    ...
    in
      rigidVertically (showClause firstParams (starter, Seq.nth innerElems 0))
  ...
  in
            ...
            case exbind of
              ExnNew {opp, id, arg} =>
                separateWithSpaces
                  [ Option.map token opp
                  , SOME (token id)
                  , Option.map (fn {off, ty} => token off ++ space ++ showTy ty)
                      arg
                  ]
    ...
    | DecAbstype {abstypee, datbind, withtypee, withh, dec, endd} =>
        let
          ...
          val bottom = group (token withh $$ indent (showDec dec) $$ token endd)
        in
          group
            (case withtypee of
               SOME {withtypee, typbind} =>
  ...
  in
    ...
    | Andalso {left, andalsoo, right} => showInfixedExp (left, andalsoo, right)
